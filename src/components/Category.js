...
export default function Category () {
    ...
    //从后台获取分类信息 并进行处理
    const getCategory = () => {
        axios.get('http://localhost:3001/category/all').then(result => {
    ...
    const deleteCategory = (record) => {
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                    await axios.delete(`http://localhost:3001/category/delete/${record}`).then(result => {
        ...
        <>
            ...
            >
                ...
                <Column key={'action'} title={'操作'}  align="center" render={(record) => (
                    <Space size={'middle'}>
                        <Button type={'link'} size={'small'} icon={<FormOutlined/>} onClick={() => {setEditCategory(record);setUpdateCategoryForm(true)}}>编辑</Button>
                        <Button  danger type='link' size={'small'} icon={<CloseOutlined/>} onClick={() =>deleteCategory(record.cat_id)}>删除</Button>
                    </Space>
                )}></Column>
            </Table>
        </>
