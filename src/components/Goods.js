...
export default function Goods() {
    ...
    //获取所有的商品信息 请求后台
    const getGoods = () => {
        axios.get('http://localhost:3001/goods/all').then( result => {
    ...
    // 根据id查询商品
    const oneGood = (value) => {
        console.log('要查询的商品id：',value);
        axios.get(`http://localhost:3001/goods/find/${value}`).then(result => {
            console.log('根据id查到的商品信息--从后台传到前端的信息: ',result.data.data);
            let arr=[]
    ...
    const deleteGoods = (id) => {
        ...
        Modal.confirm({
            ...
            onOk: () => {   //当用户点击确定按钮时执行的操作
                ...
                axios.delete(`http://localhost:3001/goods/delete/${id}`).then(result => {
                    console.log(id)
    ...
    return (
        <>
            ...
            <Input.Search placeholder="请输入待搜索的商品ID" allowClear
                    onSearch={oneGood} style={{ width: '40%',marginBottom:'10px'}} enterButton/>
            <Table
                dataSource={goods}
                border
            ...
            >
                ...
                <Column key={'action'} title={'操作'}  align="center" render={(record) => (
                    <Space size={'middle'}>
                        <Button type={'link'} size={'small'} icon={<FormOutlined/>} onClick={() => openUpdateModal(record)}>编辑</Button>
                        <Button  danger type='link' size={'small'} icon={<CloseOutlined/>} onClick={() => deleteGoods(record.goods_id)}>删除</Button>
                    </Space>
                )}></Column>
            </Table>
        </>
