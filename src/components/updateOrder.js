    ...
    return (
        <Modal
            ...
            onOk={async ()=> {
                await bindOrder.current.validateFields().then(value => {
                    axios.put('http://localhost:3001/order/update',{order:value}).then(result=> {
                        props.close()
        ...
        >
            <Form ref={bindOrder}>
                ...
                <Form.Item name='order_price' label='订单总额' initialValue={props.edit.order_price}>
                    <Input/>
                </Form.Item>
                <Form.Item name='consignee_addr' label={'地址'} initialValue={props.edit.consignee_addr}>
                    <Input/>
                </Form.Item>
